(function () {
    ...
    function formHtml(cfg) {
        ...
        return '<div class="ea-card">' +
            '<div class="ea-badge"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg></div>' +
    ...
    function render(shadow, cfg) {
        ...
        var submitLabel = TYPES[TYPE].knop;
        ...
        form.addEventListener('submit', function (e) {
            ...
            var bad = false;
            if (!raw.naam) { invalid('naam', 'Vul je naam in.'); bad = true; }
            if (raw.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(raw.email)) { invalid('email', 'Controleer je e-mailadres.'); bad = true; }
            if (!raw.email && !raw.telefoon) {
                invalid('email', 'Vul je e-mail of telefoonnummer in.');
                invalid('telefoon');
                bad = true;
            }
            if (bad) {
