(function () {
    ...
    class EazyAutomotiveWidget {
            ...
            return `
                *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                .eazy-modal-thumb {
                    ...
                    height: 48px;
                    object-fit: cover;
                    border-radius: ${Math.min(labelRadius, 6)}px;
                    cursor: pointer;
                ...
                .eazy-modal-spec-label { color: ${labelColor}; }
                .eazy-modal-spec-value { color: ${dTitleColor}; font-weight: 500; }
                .eazy-modal-opties {
                    display: flex;
                    flex-wrap: wrap;
                    gap: ${labelGap}px;
                ...
                .eazy-modal-optie {
                    ...
                    font-size: 0.8rem;
                    color: ${labelColor};
                    ${labelStyle === 'badge' ? `background: ${labelBg}; padding: 0.3rem 0.75rem; border-radius: ${labelRadius}px;` : ''}
                    ${labelStyle === 'outline' ? `background: transparent; border: 1px solid ${labelColor}; padding: 0.3rem 0.75rem; border-radius: ${labelRadius}px;` : ''}
                    ${labelStyle === 'icon-text' ? `background: transparent; padding: 0.3rem 0;` : ''}
                    ${labelStyle === 'pill' ? `background: ${labelBg}; padding: 0.3rem 0.75rem; border-radius: 9999px;` : ''}
                }
        ...
        renderDetail(overlay, car) {
            ...
            const specs = [];
            if (car.merk) specs.push(['Merk', car.merk]);
            if (car.model) specs.push(['Model', car.model]);
            ...
            modal.innerHTML = `
                ...
                <div class="eazy-modal-body">
                        ...
                        <div class="eazy-modal-specs">
                            ${specs.map(([label, val]) => `
                                <div class="eazy-modal-spec">
                                    <span class="eazy-modal-spec-label">${this.escapeHtml(label)}</span>
                    ...
                    ${car.extra_opties && car.extra_opties.length > 0 ? `
